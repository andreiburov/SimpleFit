...
#pragma omp parallel for
		for (int k = 0; k < THETA_COMPONENT_COUNT; k++)
		{
			error(k) = weight * bend_mask_per_theta_[k] * 
				expf(bend_sign_per_theta_[k] * thetas(k) - bend_bias_per_theta_[k]);
		}
	}
...
#pragma omp parallel for
		for (int k = 0; k < THETA_COMPONENT_COUNT; k++)
		{
			jacobian(k, k) = weight * bend_sign_per_theta_[k] * bend_mask_per_theta_[k] *
				expf(bend_sign_per_theta_[k] * thetas(k) - bend_bias_per_theta_[k]);
		}
	}
