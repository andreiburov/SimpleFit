...
namespace pose_reconstruction_2d
{
	...
	void TestSyntheticPoseReconstruction2D(PoseEulerCoefficients& pose)
	{
		...
		Joints joints2 = smpl_regressor(body2.vertices);
		...
		optimize.OptimizePoseFromSmplJoints2D(smpl::Optimizer::JOINT_TYPE::SMPL, shape, scaling, translation, pose);

		Body body1 = generator(shape, pose);
		Joints joints1 = smpl_regressor(body1.vertices);
		LogBodyAndProjection(body1, joints1, project, "Body1.obj", "Body1.png");
	}

	TEST_CASE("PR2D From File")
	{
		ZeroMemory(&pose, sizeof(pose));
		...
		TestSyntheticPoseReconstruction2D(pose);
	}
}